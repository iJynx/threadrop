...
class ShirtModel extends Component {
    ...
    componentDidMount() {
        ...
        const tGLTFLoader = new GLTFLoader();
        tGLTFLoader.load('/shirt.glb', function (gltf) {
            shirtMesh = gltf.scene;
            shirtMesh.scale.set(0.5, 0.5, 0.5);
            //set backwrads on y axis
            shirtMesh.rotation.y = Math.PI;
            shirtMesh.position.set(0, -1.5, 0);
            scene.add(shirtMesh);
    ...
    animate = () => {
        if (shirtMesh) shirtMesh.rotation.y += 0;
        this.renderScene();
    ...
    onProgress = xhr => {
        ...
        // USE SET STATE TO UPDATE LOADED PERCENTAGE
        this.setState({
            xhrLoaded: loaded
        });
    ...
    render() {
        return (
            <div
                className="text-xl align-center justify-center text-white align-middle h-full"
                style={{ width: "400px", height: "300px" }}
                ref={mount => {
                    this.mount = mount;
                }}
            >
                {/* {(this.state.loaded != 100) ? <span className="h-full my-auto"> Please wait to finish loading. Do not press anything in the meantime: {Math.floor(this.state.loaded) + "% loaded"}</span> : null} */}
