...
class ShirtModel extends Component {
    ...
    componentDidMount() {
        ...
        tGLTFLoader.load('/shirt.glb', function (gltf) {
            ...
            shirtMesh.scale.set(0.4, 0.4, 0.4);
            shirtMesh.position.set(0, -3, 0);
            scene.add(shirtMesh);
    ...
    animate = () => {
        if (shirtMesh) shirtMesh.rotation.y += 0.007;
        this.renderScene();
    ...
    render() {
        return (
            <div
                className="text-4xl align-center justify-center text-white align-middle h-full"
                style={{ width: "500px", height: "600px" }}
                ref={mount => {
                    this.mount = mount;
                }}
            > 
                {/* {(this.state.loaded != 100) ? <span className="h-full my-auto"><br /><br /><br /><br /><br /> Please wait to finish loading. Do not press anything in the meantime: {Math.floor(this.state.loaded) + "% loaded"}</span> : null} */}
